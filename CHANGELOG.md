...
- **Entry-eligibility store** with three states (verified-eligible | verified-ineligible | unchecked), each verified state requiring a source and an as-of date. `assert_proposable` refuses anything not verified-eligible with no third outcome, while `display()` still returns unchecked rows so a UI can label them. Passing `max_age_days` refuses a stale verification instead of letting it quietly become a lie. An as-of date in the FUTURE is refused at both ends — at write time, where the typo can still be corrected, and again at the gate, because a negative age can never trip the staleness test and would otherwise read as permanently fresh.
- **The eligibility gate is a chokepoint, not a helper.** `guard_proposed_countries` runs on every surface that PROPOSES a country: a `candidate` segment on the way in (`save_itinerary`, `add_itinerary_segment`) and any plan that counts candidates on the way out (`itinerary_open_windows`, `plan_window_packing` with `include_candidates`). `locked` and `planned` segments are facts about a trip already committed to and pass through untouched, and display paths keep rendering every state labelled. A refusal is audited as `error_class: "policy_refusal"` — a recorded no is neither malformed input nor an upstream fault.
- **Overlap detection compares every pair, not just neighbours.** One long stay can swallow several later ones, so reporting only the adjacent pair would let a caller fix every conflict it was shown and still be double-booked. Reported overlap is clipped to the shorter stay.
- **Prerequisite graph scheduled backward from departure.** `complete_by = min(departure − hard_cutoff_days, earliest start of anything depending on this)`; `start_by = complete_by − lead_days`. Dependency cycles, unknown keys and duplicate keys all raise at write time, so an unschedulable graph never reaches disk — a duplicate names the offending key rather than reporting a cycle with nothing stuck in it.
...
- `audit.classify_error` dispatches on the exception MRO rather than the leaf type name, so a new exception type inherits its base's class instead of falling through to `upstream_error` on a surface that makes no upstream call.
- Test fixtures use neutral placeholders and DAY OFFSETS from a synthetic base date. The 21/13/50 open-window vector is a property of the arithmetic, so no fixture needs a real place or a real travel date.
- Loading coerces dates defensively: an unquoted `YYYY-MM-DD` in hand-edited YAML parses as a `date` object, not a string, so every load path coerces rather than trusting the writer's quoting.
